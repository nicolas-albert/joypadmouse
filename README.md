...
- **Panic kill (default)**: **Back/Select + RB + RT (hold 3s)** (runs `joypadmouse-kill-top`)
...
- `JOYPADMOUSE_PANIC_CHORD` (default `back+rb+rt`) — panic chord (can include `rt` for the right trigger)
- `JOYPADMOUSE_PANIC_HOLD_MS` (default `3000`) — hold time for the panic chord
...
Advanced options (MangoHud pre-key, delays, hold time, deadzone, poll rate) are still available when you run `joypadmouse` directly — see `joypadmouse --help`.
